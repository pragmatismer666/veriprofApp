...
var ProfessionalPage = /** @class */ (function () {
    ...
    ProfessionalPage.prototype.update = function (key) {
        ...
        if (key == "update") {
            ...
            else if (this.account.newpass < 5) {
                this.restApi.toast("Password must be longer than 5 letters.", 1200);
                checkFlag = false;
                return;
            }
            else if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.account.email) != true) {
                this.restApi.toast("Please input validated email.", 1200);
                checkFlag = false;
                return;
            }
            else if (this.account.name < 5) {
                this.restApi.toast("Name must be longer than 5 letters.", 1200);
                checkFlag = false;
                return;
            }
            else if (this.account.mobile <= 10) {
                this.restApi.toast("Phone Number must be longer than 10 numbers.", 1200);
                checkFlag = false;
                return;
            }
        }
    ...
    ProfessionalPage.prototype.checkStaff = function () {
        ...
        this.restApi.post('professional/check-staff', { email: this.account.email }).subscribe(function (res) {
            if (res && res.status == "success") {
                if (res.data == 'no') {
                    _this.restApi.toast("There is no staff confirm request.", 1200);
                }

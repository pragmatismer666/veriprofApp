...
var ProfilePage = /** @class */ (function () {
    ...
    ProfilePage.prototype.addsoft = function () {
        var new_soft = "<ion-item><ion-label position='stacked' color='success'>Software No : " + (this.soft_id + 1).toString() + "</ion-label><ion-input id='soft" + this.soft_id.toString() + "' [(ngModel)]='softs[" + this.soft_id + "]'></ion-input></ion-item><ion-item><ion-label position='stacked'>Upload Pdf to proof the address</ion-label><ion-input name='file' type='file' accept='application/pdf' id='soft" + this.soft_id.toString() + "'></ion-input></ion-item><br>";
    ...
    ProfilePage.prototype.addhard = function () {
        var new_hard = "<ion-item><ion-label position='stacked' color='success'>Hardware No : " + (this.hard_id + 1).toString() + "</ion-label><ion-input id='hard" + this.hard_id.toString() + "'  [(ngModel)]='hards[" + this.hard_id + "]'></ion-input></ion-item><ion-item><ion-label position='stacked'>Upload Pdf to proof the address</ion-label><ion-input name='file' type='file' accept='application/pdf' id='hard" + this.hard_id.toString() + "'></ion-input></ion-item><br>";
    ...
    ProfilePage.prototype.manage = function () {
        ...
        for (var i = 0; i < this.soft_id; i++) {
            ...
            var name = document.getElementById("soft" + i.toString()).value.toString();
            if (name == null || name == '') {
        ...
        for (var i = 0; i < this.hard_id; i++) {
            ...
            var name = document.getElementById("hard" + i.toString()).value.toString();
            if (name == null || name == '') {
    ...
    ProfilePage.prototype.addBusiness = function () {
        ...
        for (var x in this.pform) {
            if (x == 'verified_at' || x == 'verified_by')
                continue;
        ...
        if (this.is_director == true && event.detail.value == "business") {
            this.restApi.post('professional/get-list', { user_id: this.authService.user.userId }).subscribe(function (res) {
                if (res && res.status) {
                    ...
                    _this.mybusiness = res.data[2].buzi_id;
                    _this.myoffice = res.data[2].office_id;
                }
            }, function (error) {
                _this.toastController.create({
                    message: 'Something went wrong.',

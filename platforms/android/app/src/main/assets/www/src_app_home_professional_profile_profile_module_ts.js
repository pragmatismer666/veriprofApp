...
let ProfilePage = class ProfilePage {
    ...
    addStaff() {
        ...
        document.getElementById(`ho_staff_prof_registration${this.ho_staff_id.toString()}`).addEventListener("change", (e) => { this.changeListener_dynamic(e, `ho_staff_prof_registration${this.ho_staff_id.toString()}`); });
        document.getElementById(`ho_staff_qualification${this.ho_staff_id.toString()}`).addEventListener("change", (e) => { this.changeListener_dynamic(e, `ho_staff_qualification${this.ho_staff_id.toString()}`); });
    ...
    manage() {
        ...
        for (let i = 0; i < this.director_id; i++) {
            if (document.getElementById(`director${i}`) == null) {
                ...
                let name = document.getElementById("dir_name" + i.toString()).value.toString();
                var prof_reg_no = document.getElementById("dir_prof_reg_no" + i.toString()).value.toString();
                var owned = document.getElementById("dir_owned" + i.toString()).value.toString();
        ...
        for (let i = 0; i < this.ho_staff_id; i++) {
            try {
                if (document.getElementById(`hstaff${i}`) == null) {
                    ...
                    let file_cv = "";
                    let file_prof_registration = "";
                    let file_qualification = "";
                    for (let j = 0; j < this.pdfs.length; j++) {
                        ...
                        if ("file_prof_registration" == this.pdfs[j].name && (i + 1).toString() == this.pdfs[j].id.toString()) {
                            file_prof_registration = this.pdfs[j].filename;
                        }
                        if ("file_qualification" == this.pdfs[j].name && (i + 1).toString() == this.pdfs[j].id.toString()) {
                            file_qualification = this.pdfs[j].filename;
                        }
                    }
    ...
    addBusiness() {
        ...
        if (result_manage.status) {
            ...
            this.restApi.post("professional/add-business", { user_id: this.authService.user.userId, data: this.pform, hoffice: this.hform, softs: this.softs, hards: this.hards, staffs: this.staffs, branchs: this.bo_offices, directors: this.directors }).subscribe((res) => {
                console.log(res);
                if (res && res.status) {
    ...
    addbstaff(id, office_id) {
        ...
        try {
            ...
            document.getElementById(`bo_staff_prof_registration${this.bo_staff_id.toString()}`).addEventListener("change", (e) => { this.changeListener_dynamic(e, `bo_staff_prof_registration${this.bo_staff_id.toString()}`); });
            document.getElementById(`bo_staff_qualification${this.bo_staff_id.toString()}`).addEventListener("change", (e) => { this.changeListener_dynamic(e, `bo_staff_qualification${this.bo_staff_id.toString()}`); });
        }
        catch (error) {
            ...
            for (let j = 0; j < eachOffice.length; j++) {
                try {
                    ...
                    for (let k = 0; k < this.pdfs.length; k++) {
                        ...
                        if ("bo_file_prof_registration" == this.pdfs[k].name && (bo_staff_id + 1).toString() == this.pdfs[k].id.toString()) {
                            file_prof_registration = this.pdfs[k].filename;
                        }
                        if ("bo_file_qualification" == this.pdfs[k].name && (bo_staff_id + 1).toString() == this.pdfs[k].id.toString()) {
                            file_qualification = this.pdfs[k].filename;
                        }
                    }
                    if (file_appointment == "") {
